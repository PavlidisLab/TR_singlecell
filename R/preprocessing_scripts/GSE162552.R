...
# Files were directly downloaded from GEO, see GSE162552_download.sh in dat_dir
...
if (!file.exists(processed_path)) {
  ...
  change_colnames <- c(Cell_type = "SubregionReAnnot")
  
  meta <- dat[[]] %>% 
    dplyr::rename(any_of(change_colnames)) %>% 
    mutate(
      assay = "Smart-seq2",
      Cell_type = paste0("vGlut1Crepos_neurons_", Cell_type)) %>% 
