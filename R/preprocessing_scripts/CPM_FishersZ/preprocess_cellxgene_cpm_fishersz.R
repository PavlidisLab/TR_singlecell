...
aggmat_path <- file.path(out_dir, paste0(id, "_FishersZ_CPM.tsv"))
...
if (!file.exists(aggmat_path)) {
  
  agg <- fishersZ_aggregate(mat, meta)
  
  # Write as data.frames (preserve rownames) with data.table fwrite (fast)
  
  fwrite(
    data.frame(agg$Agg_mat, check.names = FALSE),
    sep = "\t",
    row.names = TRUE,
    quote = FALSE,
    verbose = FALSE,
  ...
  fwrite(
    data.frame(agg$NA_mat, check.names = FALSE),
    sep = "\t",
    row.names = TRUE,
    quote = FALSE,
    verbose = FALSE,
