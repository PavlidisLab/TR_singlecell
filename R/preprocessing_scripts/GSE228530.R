...
# Files were directly downloaded from GEO, see GSE228530_download.sh in dat_dir
dat_path <- list.files(dat_dir, pattern = "count", full.names = TRUE)
...
if (!file.exists(processed_path)) {
  ...
  meta$X <- NULL
  meta <- data.frame(ID = str_replace(meta$Barcode, "_[:digit:]$", ""), meta)
  ...
  meta <- filter(meta, !str_detect(Cell_type, "Unknown") & !is.na(Cell_type))
  mat <- mat[, meta$ID]
