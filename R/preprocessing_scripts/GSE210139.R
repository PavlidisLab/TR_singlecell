...
# Files were directly downloaded from GEO, see GSE210139_download.sh in dat_dir
...
if (!file.exists(processed_path)) {
  ...
  dat_l <- lapply(dat_path, readRDS)
  dat <- Reduce(merge, dat_l)
  ...
  # Confirming that duplicated cell names are not identical
  
  ids_l <- lapply(dat_l, colnames)
  ids <- unlist(ids_l)
  dupl <- ids[duplicated(ids)]
  
  cor_l <- lapply(dupl, function(x) {
    ids <- colnames(mat)[which(str_detect(colnames(mat), x))]
    cor(mat[, ids], method = "spearman")
  })
  
  
  any(vapply(cor_l, function(x) {
    diag(x) <- NA
    any(x == 1, na.rm = TRUE)
  }, FUN.VALUE = logical(1)))
  ...
  change_colnames <- c(Cell_type = "Celltypes")
