...
# Files were directly downloaded from GEO, see GSE123335_download.sh in dat_dir
...
if (!file.exists(processed_path)) {
  ...
  common <- intersect(rownames(mat_l[[1]]), rownames(mat_l[[2]]))
  mat <- do.call(cbind, lapply(mat_l, function(x) x[common, ]))
  mat <- mat[, meta$CellID]
  ...
  # Ready metadata
  # "GSE123335" collapse cell types
  
  change_colnames <- c(Cell_type = "Cluster", ID = "CellID")
  
  meta <- meta %>%
    dplyr::rename(any_of(change_colnames)) %>%
    mutate(
      assay = "Drop-seq",
      Cell_type = trimws(str_replace(Cell_type, "\\[.*\\]$", "")),
      Cell_type = trimws(str_replace(Cell_type, "[:digit:]$", ""))
    )  %>%
