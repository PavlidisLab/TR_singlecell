...
# Files were directly downloaded from GEO, see GSE142645Mouse_download.sh in dat_dir
...
if (!file.exists(processed_path)) {
  ...
  # "GSE142645Mouse" had to directly access counts
  
  mat <- dat@assays$RNA@counts
  
  
  # Ready metadata
  # "GSE142645Mouse" collapse macrophages cell types
  
  change_colnames <- c(Cell_type = "new_anno4")
  
  meta <- dat[[]] %>% 
    dplyr::rename(any_of(change_colnames)) %>% 
    mutate(
      assay = "10x 3' v2",
      Cell_type = str_replace(Cell_type, " I$| II$", "")
      ) %>% 
