...
# Files were directly downloaded from GEO, see GSE180041_download.sh in dat_dir
# dat_path <- file.path(dat_dir, paste0(id, "_counts.csv"))
dat_path <- "~/sc_input/test/GSE180041_exonic_norm_umi.csv"
meta_path <- file.path(dat_dir, paste0(id, "_metadata.csv"))

mat <- read.csv(dat_path)
dim(mat)
mat[1:10, 1:10]
stop()
...
if (!file.exists(processed_path)) {
  ...
  change_colnames <- c(Cell_type = "cluster_interpretation", ID = "V1")
