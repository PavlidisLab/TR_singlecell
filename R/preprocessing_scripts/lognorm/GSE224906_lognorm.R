...
# Files were directly downloaded from GEO, see GSE224906_download.sh in dat_dir
...
if (!file.exists(processed_path)) {
  ...
  meta <- data.frame(ID = str_replace(rownames(meta), " ", "_"), meta)
  
  mat <- read_count_mat(dat_path)
  colnames(mat) <- str_replace(colnames(mat), "\\.", "_")
  mat <- mat[, meta$ID]
