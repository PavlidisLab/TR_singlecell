...
# Files were directly downloaded from GEO, see GSE160512_download.sh in dat_dir
dat_path <- list.files(dat_dir, pattern = "SAM24", full.names = TRUE)
...
if (!file.exists(processed_path)) {
  ...
  change_colnames <- c(Cell_type = "allCells.cluster.interpretation", ID = "cellID")
