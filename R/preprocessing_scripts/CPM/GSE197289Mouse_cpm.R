...
# Files were directly downloaded from GEO, see GSE197289Mouse_download.sh in dat_dir
dat_path <- file.path(dat_dir, paste0(id, "_counts.RDS"))
...
if (!file.exists(processed_path)) {
  ...
  # Load metadata and the count matrix. Remove duplicate IDs in meta
  
  meta <- as.data.frame(fread(meta_path))
  meta <- filter(meta, !duplicated(V1))
  
  mat <- readRDS(dat_path)
  mat <- mat[, meta$V1]
  ...
  change_colnames <- c(Cell_type = "cellID", ID = "V1")
