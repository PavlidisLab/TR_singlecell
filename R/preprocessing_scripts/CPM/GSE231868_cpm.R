...
# Files were directly downloaded from GEO, see GSE231868_download.sh in dat_dir
dat_path <- list.files(dat_dir, pattern = "mtx", full.names = TRUE)
...
if (!file.exists(processed_path)) {
  ...
  meta <- data.frame(ID = str_replace(meta$cell_barcode, "_[:digit:]$", ""), meta)
  ...
  change_colnames <- c(Cell_type = "type_name")
