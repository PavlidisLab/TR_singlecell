...
# Files were directly downloaded from GEO, see GSE163120_download.sh in dat_dir
meta_path <- file.path(dat_dir, paste0(id, "_metadata.csv"))
mat_path <- file.path(dat_dir, paste0(id, "_counts.csv"))
...
if (!file.exists(processed_path)) {
  ...
  stopifnot(all(colnames(mat) %in% meta$cell))
  
  
  # Ready metadata
  # "GSE163120" collapse cell types with integer delimiter
