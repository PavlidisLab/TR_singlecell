...
# Files were directly downloaded from GEO, see GSE145172_download.sh in dat_dir
...
if (!file.exists(processed_path)) {
  ...
  meta <- as.data.frame(fread(meta_path, check.names = FALSE))
  mat <- t(Matrix::readMM(mat_path))  # note t()
  ...
  change_colnames <- c(Cell_type = "celltype", ID = "barcode")
