...
# Files were directly downloaded from GEO, see GSE168038_download.sh in dat_dir
meta_path <- file.path(dat_dir, paste0(id, "_metadata.txt"))
mat_path <- list.files(dat_dir, pattern = "matrix", full.names = TRUE)
...
if (!file.exists(processed_path)) {
  ...
  meta <- data.frame(ID = str_replace(meta$V1, "^.*_.*_", ""), meta)
  ...
  # Ready metadata
  # "GSE168038" collapse cell types and remove ambiguous
  
  change_colnames <- c(Cell_type = "cluster_name")
  ...
  meta <- filter(meta, !(Cell_type %in% c("other", "low_quality_cells")))
  mat <- mat[, meta$ID]
