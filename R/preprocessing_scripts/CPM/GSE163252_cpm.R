...
# Files were directly downloaded from GEO, see GSE163252_download.sh in dat_dir
meta_path <- file.path(dat_dir, paste0(id, "_metadata.txt"))
mat_path <- file.path(dat_dir, paste0(id, "_counts.txt"))
...
if (!file.exists(processed_path)) {
  ...
  # Row/gene names have chromosome info of variable length appended - remove
  
  genes1 <- rownames(mat)
  genes2 <- str_replace(genes1, "-[:digit:]+-[-|+]-[:digit:]", "")
  genes3 <- str_replace(genes2, "-[X|Y]+-[-|+]-[:digit:]", "")
  
  dupl <- genes2[duplicated(genes3) | duplicated(genes3, fromLast = TRUE)]
  which_dupl <- which(genes3 %in% dupl)
  genes4 <- genes3[-which_dupl]
  
  mat2 <- mat[-which_dupl, ]
  rownames(mat2) <- genes4
  
  stopifnot(identical(as.integer(mat2["Actb", ]), as.integer(mat["Actb-5---1",])))
  mat <- mat2

  # Ready metadata
  
  change_colnames <- c(Cell_type = "x")
