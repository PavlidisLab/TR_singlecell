...
# Files were directly downloaded from GEO, see GSE104323_download.sh in dat_dir
dat_path <- file.path(dat_dir, paste0(id, "_counts.tab"))
...
if (!file.exists(processed_path)) {
  ...
  mat <- read.delim(dat_path)
  rownames(mat) <- mat$cellid
  mat$cellid <- NULL
  mat <- as.matrix(mat)
  colnames(mat) <- str_replace(colnames(mat), "\\.$", "-")
  colnames(mat) <- str_replace(colnames(mat), "^X", "")
  
  common <- intersect(colnames(mat), meta$`Sample name (24185 single cells)`)
  mat <- mat[, common]
  
  meta <- meta %>% 
    filter(`Sample name (24185 single cells)` %in% common) %>% 
    arrange(match(`Sample name (24185 single cells)`, common))
  
  stopifnot(identical(colnames(mat), meta$`Sample name (24185 single cells)`))
  

  # Ready metadata
  
  change_colnames <- c(Cell_type = "characteristics: cell cluster", ID = "Sample name (24185 single cells)")
