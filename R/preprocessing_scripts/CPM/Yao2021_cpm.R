...
# Downloaded from cxg but need to remove duplicated cells from Scala2021
...
if (!file.exists(processed_path)) {
  ...
  # Ready metadata
  # "Yao2021" Remove duplicated ID
  
  change_colnames <- c(Cell_type = "cell_type", ID = "V1")
  
  meta <- dat[[]] %>% 
    dplyr::rename(any_of(change_colnames)) %>% 
    mutate(assay = "10x 3' v2/v3, Smart-seq") %>% 
    rownames_to_column(var = "ID") %>% 
    filter(!(donor_id %in% dupl_ids[[1]]))
