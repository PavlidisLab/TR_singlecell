...
# Files were directly downloaded from GEO, see GSE144317_download.sh in dat_dir
dat_path <- list.files(dat_dir, pattern = "GSM42", full.names = TRUE)
...
if (!file.exists(processed_path)) {
  ...
  # Ready metadata
  # "GSE144317" collapse cell types delimited by I/II
  
  change_colnames <- c(Cell_type = "annotation", ID = "cell")
  
  meta <- meta %>% 
    dplyr::rename(any_of(change_colnames)) %>% 
    mutate(
      assay = "MARS-seq",
      Cell_type = str_replace(Cell_type, " [I]+", "")) %>% 
