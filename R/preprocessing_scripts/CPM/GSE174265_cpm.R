...
# Files were directly downloaded from GEO, see GSE174265_download.sh in dat_dir
dat_path <- file.path(dat_dir, paste0(id, "_counts.tsv"))
...
if (!file.exists(processed_path)) {
  ...
  mat <- as.matrix(read.delim(dat_path, header = FALSE))
  barcodes <- read.delim(barcodes_path, header = FALSE)
  features <- read.delim(features_path, header = FALSE)
  
  dupl <- which(duplicated(features$V1))
  features <- features[-dupl, "V1", drop = FALSE]
  mat <- mat[-dupl, ]
  
  stopifnot(identical(barcodes$V1, str_replace(meta$fastq1, "_1_sequence.fastq", "")))
  meta <- data.frame(ID = barcodes$V1, meta)
  ...
  change_colnames <- c(Cell_type = "celltype")
  
  meta <- meta %>% 
    dplyr::rename(any_of(change_colnames)) %>% 
    mutate(assay = "Smart-seq2") %>% 
