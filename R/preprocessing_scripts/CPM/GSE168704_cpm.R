...
# Files were directly downloaded from GEO, see GSE168704_download.sh in dat_dir
...
if (!file.exists(processed_path)) {
  ...
  meta_l[[1]] <- meta_l[[1]] %>%
    dplyr::rename(ID = "cellBC",
                  Ventricle = "ventricle",
                  Cell_type = "cell.id") %>%
    mutate(Age = NA) %>%
    select(ID, Ventricle, Cell_type, Age)
    
    
  meta_l[[2]] <- meta_l[[2]] %>%
    dplyr::rename(ID = "cellBC", Cell_type = "Cell.IDs") %>%
    mutate(Cell_type = case_when(
      Cell_type == "Endo" ~ "Endothelial",
      Cell_type == "Epi" ~ "Epithelial",
      TRUE ~ Cell_type)
      ) %>%
    select(ID, Ventricle, Cell_type, Age)
