...
avg_coexpr_hg_path <- "/space/scratch/amorin/R_objects/avg_coexpr_hg_lognorm.RDS"
avg_coexpr_mm_path <- "/space/scratch/amorin/R_objects/avg_coexpr_mm_lognorm.RDS"
...
get_avg_coexpr <- function(ids, genes) {
  ...
  for (id in ids) {
    mat <- load_agg_mat_list(id, genes = genes, pattern = "_RSR_allrank.tsv")[[1]]
    avg_mat <- avg_mat + mat
  }
...
outfile <- "/space/scratch/amorin/R_objects/top_cor_pair_list_lognorm.RDS"
